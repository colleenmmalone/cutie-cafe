...
function useRoutesImpl(routes2, locationArg, dataRouterState, onError, future2) {
  invariant$1(
    useInRouterContext(),
    // TODO: This error is probably because they somehow have 2 versions of the
    // router loaded. We can help them understand how to avoid that.
...
function _renderMatches(matches, parentMatches = [], dataRouterState = null, onErrorHandler = null, future2 = null) {
  if (matches == null) {
    if (!dataRouterState) {
      return null;
    }
...
function DataRoutes({
  ...
  return useRoutesImpl(routes2, void 0, state, onError, future2);
}
...
function getManifestPath(_manifestPath, basename2) {
  ...
  return `${basename2}${manifestPath}`.replace(/\/+/g, "/");
}
...
function PrefetchPageLinksImpl({
  ...
  let dataHrefs = reactExports.useMemo(() => {
    ...
    let url = singleFetchUrl(
      page,
      basename2,
      future2.unstable_trailingSlashAwareDataRequests,
      "data"
    );
...
function DataRoutes2({
  ...
  return useRoutesImpl(routes2, void 0, state, void 0, future2);
}
...
function getDocumentHeaders(context, build2) {
  return getDocumentHeadersImpl(context, (m) => {
    let route = build2.routes[m.route.id];
...
async function singleFetchAction(build2, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
  try {
    throwIfPotentialCSRFAttack(
      request.headers,
      Array.isArray(build2.allowedActionOrigins) ? build2.allowedActionOrigins : []
    );
  ...
  function handleQueryError(error) {
    handleError(error);
    return generateSingleFetchResponse(request, build2, serverMode, {
      result: { error },
      headers: new Headers(),
      status: 500
    });
  ...
  function staticContextToResponse(context) {
    let headers = getDocumentHeaders(context, build2);
...
async function singleFetchLoaders(build2, serverMode, staticHandler, request, handlerUrl, loadContext, handleError) {
...
function generateSingleFetchResponse(request, build2, serverMode, {
  result,
  headers,
  status
}) {
  ...
  return new Response(
    encodeViaTurboStream(
      result,
      request.signal,
      build2.entry.module.streamTimeout,
      serverMode
    ),
    {
      status: status || 200,
...
function generateSingleFetchRedirectResponse(redirectResponse, request, build2, serverMode) {
...
function derive(build2, mode) {
  let routes2 = createRoutes(build2.routes);
  let dataRoutes = createStaticHandlerDataRoutes(build2.routes, build2.future);
  let serverMode = isServerMode(mode) ? mode : "production";
  let staticHandler = createStaticHandler(dataRoutes, {
    basename: build2.basename,
    unstable_instrumentations: build2.entry.module.unstable_instrumentations
  });
  let errorHandler = build2.entry.module.handleError || ((error, { request }) => {
  ...
  let requestHandler = async (request, initialContext) => {
    ...
    if (build2.future.v8_middleware) {
    ...
    let url = new URL(request.url);
    let normalizedBasename = build2.basename || "/";
    let normalizedPath = url.pathname;
    if (build2.future.unstable_trailingSlashAwareDataRequests) {
    ...
    if (!build2.ssr) {
      ...
      if (build2.prerender.length === 0) {
        isSpaMode2 = true;
      } else if (!build2.prerender.includes(decodedPath) && !build2.prerender.includes(decodedPath + "/")) {
    ...
    let manifestUrl = getManifestPath(
      build2.routeDiscovery.manifestPath,
      normalizedBasename
    );
    if (url.pathname === manifestUrl) {
      try {
        let res = await handleManifestRequest(build2, routes2, url);
        return res;
      } catch (e) {
        handleError(e);
    ...
    let matches = matchServerRoutes(routes2, normalizedPath, build2.basename);
    ...
    if (url.pathname.endsWith(".data")) {
      ...
      if (build2.entry.module.handleDataRequest) {
        response = await build2.entry.module.handleDataRequest(response, {
          context: loadContext,
      ...
      if (build2.unstable_getCriticalCss) {
        criticalCss = await build2.unstable_getCriticalCss({ pathname });
  ...
  if (build2.entry.module.unstable_instrumentations) {
    requestHandler = instrumentHandler(
      requestHandler,
      build2.entry.module.unstable_instrumentations.map((i) => i.handler).filter(Boolean)
    );
  }
  return {
    routes: routes2,
...
var createRequestHandler = (build2, mode) => {
  let _build;
  let routes2;
  let serverMode;
  let staticHandler;
  ...
  return async function requestHandler(request, initialContext) {
    ...
    } else if (!routes2 || !serverMode || !staticHandler || !errorHandler || !_requestHandler) {
      let derived = derive(_build, mode);
      routes2 = derived.routes;
      serverMode = derived.serverMode;
      staticHandler = derived.staticHandler;
...
async function handleManifestRequest(build2, routes2, url) {
  if (build2.assets.version !== url.searchParams.get("version")) {
    return new Response(null, {
      status: 204,
      headers: {
  ...
  if (url.searchParams.has("paths")) {
    ...
    for (let path of paths) {
      let matches = matchServerRoutes(routes2, path, build2.basename);
      if (matches) {
        for (let match of matches) {
          let routeId = match.route.id;
          let route = build2.assets.routes[routeId];
          if (route) {
...
async function handleSingleFetchRequest(serverMode, build2, staticHandler, request, handlerUrl, loadContext, handleError) {
...
async function handleDocumentRequest(serverMode, build2, staticHandler, request, loadContext, handleError, isSpaMode2, criticalCss) {
  try {
  ...
  async function renderHtml(context, isSpaMode22) {
    ...
    let baseServerHandoff = {
      ...
      routeDiscovery: build2.routeDiscovery,
      ssr: build2.ssr,
      isSpaMode: isSpaMode22
    };
    let entryContext = {
      manifest: build2.assets,
      routeModules: createEntryRouteModules(build2.routes),
...
async function handleResourceRequest(serverMode, build2, staticHandler, routeId, request, loadContext, handleError) {
  try {
...
async function handleRequest(request, responseStatusCode, responseHeaders, routerContext, _loadContext) {
  ...
  const body = await server_browserExports.renderToReadableStream(/* @__PURE__ */ jsxRuntimeExports.jsx(ServerRouter, { context: routerContext, url: request.url }), {
    onError(error) {
...
function Footer() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full min-h-[300px] flex flex-1 flex-row items-left justify-between bg-[#bb99ff] py-2 px-8", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full min-h-[300px] flex flex-col items-left justify-center gap-6", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-white text-2xl", children: "Cute Cafe" }),
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-1 items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "text-9xl", children: "☕" }) })
  ] });
}
function NavTop() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "w-full flex flex-1 sm:flex-row flex-col items-center justify-between bg-[#bb99ff] py-2 px-8", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-white text-2xl", children: "Cute Cafe" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "flex flex-row space-x-4 ", children: resources$1.map(({ href, text }) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      NavLink,
      {
...
const layout = withComponentProps(function Layout2() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", {
    ...
    children: [/* @__PURE__ */ jsxRuntimeExports.jsx(NavTop, {}), /* @__PURE__ */ jsxRuntimeExports.jsx(Outlet, {}), /* @__PURE__ */ jsxRuntimeExports.jsx(Footer, {})]
  });
});
...
function Hero() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-full m-0 p-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "hero-splash h-[calc(100vh-95px)] sm:h-[calc(100vh-64px)] w-screen p-0 m-0 bg-cover bg-center overflow-scroll", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "h-[200%] flex items-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-white text-4xl text-center mt-auto mb-[50vh] mx-auto border-1 rounded-lg py-2 px-6 bg-[#3079]", children: "Where caffeine meets cute!" }) }) }) });
}
function Card(props) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 flex flex-col items-center gap-12 p-6 py-16 min-h-0 bg-white my-16 min-w-[80%] rounded-3xl", children: props.children });
}
function BtnSeeMenu() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Link, { to: "/menu", children: /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "rounded-xl bg-[#880088] text-white py-4 px-6", children: "See the Menu" }) });
}
function Welcome() {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "flex flex-col items-center justify-center pt-0 mt-0 pb-4", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(Hero, {}),
    /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, { children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "We can't wait to meet you!" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(BtnSeeMenu, {})
    ] })
  ] });
}
function meta$2({}) {
  return [{
    ...
    name: "description",
    content: "Where caffeine meets cute!"
  }];
}
...
function PageTitle(props) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-full items-left justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { className: "text-red text-3xl font-bold", children: props.text }) });
}
const barista = "/assets/barista-BxlOgb5S.jpg";
function meta$1({}) {
  return [{
    ...
    children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", {
      ...
      children: [/* @__PURE__ */ jsxRuntimeExports.jsx("header", {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", {
          ...
          children: /* @__PURE__ */ jsxRuntimeExports.jsx(PageTitle, {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Card, {
          children: [/* @__PURE__ */ jsxRuntimeExports.jsx("h3", {
            ...
            children: ["Hairy is our top-notch always-friendly barista! With over 8 years of experience, he has traveled the globe, searching for the highest quality, best-tasting coffee to bring home to you. No wonder he is employee of the month every month!", /* @__PURE__ */ jsxRuntimeExports.jsx("br", {}), /* @__PURE__ */ jsxRuntimeExports.jsx("br", {}), "Hurry in to our shop to experience the world's best coffee for yourself!"]
          }), /* @__PURE__ */ jsxRuntimeExports.jsx(BtnSeeMenu, {})]
        })
      })]
    })
...
function MenuCard({ props }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white rounded-xl w-full h-full p-3 flex sm:flex-col flex-row items-center justify-center text-center", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "min-w-[90px]", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-7xl", children: [
      " ",
      props.icon
    ] }) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "font-semibold ml-4 sm:ml-0", children: [
      " ",
      props.text
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs ml-auto sm:mx-auto", children: [
      "$",
      props.price
    ] })
  ] }) });
...
function MenuMap() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-full items-center justify-center", children: /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: " w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mx-auto", children: resources.map((m, i) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: /* @__PURE__ */ jsxRuntimeExports.jsx(MenuCard, { props: m }) }, i)) }) });
}
const resources = [
  {
    text: "Espresso",
    price: 1.99,
    icon: /* @__PURE__ */ jsxRuntimeExports.jsx(jsxRuntimeExports.Fragment, { children: "☃" })
  },
  {
    text: "Latte",
    price: 5.99,
...
const menu = withComponentProps(function Menu() {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("main", {
    ...
    children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", {
      ...
      children: [/* @__PURE__ */ jsxRuntimeExports.jsx("header", {
        ...
        children: /* @__PURE__ */ jsxRuntimeExports.jsx(MenuMap, {})
      })]
    })
  });
});
...
const serverManifest = { "entry": { "module": "/assets/entry.client-LfwCoNJB.js", "imports": ["/assets/chunk-EPOLDU6W-DI_FKu8q.js"], "css": [] }, "routes": { "root": { "id": "root", "parentId": void 0, "path": "", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasClientMiddleware": false, "hasErrorBoundary": true, "module": "/assets/root-CwOVyWog.js", "imports": ["/assets/chunk-EPOLDU6W-DI_FKu8q.js"], "css": ["/assets/root-CCW8L8N6.css"], "clientActionModule": void 0, "clientLoaderModule": void 0, "clientMiddlewareModule": void 0, "hydrateFallbackModule": void 0 }, "routes/layout": { "id": "routes/layout", "parentId": "root", "path": void 0, "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasClientMiddleware": false, "hasErrorBoundary": false, "module": "/assets/layout-C0wHOhKB.js", "imports": ["/assets/chunk-EPOLDU6W-DI_FKu8q.js"], "css": [], "clientActionModule": void 0, "clientLoaderModule": void 0, "clientMiddlewareModule": void 0, "hydrateFallbackModule": void 0 }, "routes/home": { "id": "routes/home", "parentId": "routes/layout", "path": void 0, "index": true, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasClientMiddleware": false, "hasErrorBoundary": false, "module": "/assets/home-gM1FEO3i.js", "imports": ["/assets/chunk-EPOLDU6W-DI_FKu8q.js", "/assets/btn-seemenu-CsawUTR2.js"], "css": [], "clientActionModule": void 0, "clientLoaderModule": void 0, "clientMiddlewareModule": void 0, "hydrateFallbackModule": void 0 }, "routes/about": { "id": "routes/about", "parentId": "routes/layout", "path": "about", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasClientMiddleware": false, "hasErrorBoundary": false, "module": "/assets/about-CsGXhCXp.js", "imports": ["/assets/chunk-EPOLDU6W-DI_FKu8q.js", "/assets/pagetitle-q9cWpAch.js", "/assets/btn-seemenu-CsawUTR2.js"], "css": [], "clientActionModule": void 0, "clientLoaderModule": void 0, "clientMiddlewareModule": void 0, "hydrateFallbackModule": void 0 }, "routes/menu": { "id": "routes/menu", "parentId": "routes/layout", "path": "menu", "index": void 0, "caseSensitive": void 0, "hasAction": false, "hasLoader": false, "hasClientAction": false, "hasClientLoader": false, "hasClientMiddleware": false, "hasErrorBoundary": false, "module": "/assets/menu-CMBY7i9a.js", "imports": ["/assets/chunk-EPOLDU6W-DI_FKu8q.js", "/assets/pagetitle-q9cWpAch.js"], "css": [], "clientActionModule": void 0, "clientLoaderModule": void 0, "clientMiddlewareModule": void 0, "hydrateFallbackModule": void 0 } }, "url": "/assets/manifest-9abe7a22.js", "version": "9abe7a22", "sri": void 0 };
...
export {
  RouterContextProvider as R,
  createRequestHandler as a,
  build as b,
  createContext as c,
  allowedActionOrigins as d,
  assetsBuildDirectory as e,
  basename as f,
  entry as g,
  future as h,
  isSpaMode as i,
